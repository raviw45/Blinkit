...
export const createOrder=async(req,reply)=>{
     try {
        ...
        return reply.status(500).send({message:"Failed to create the order",error});
     }
}
...
export const confirmOrder=async(req,reply)=>{
   try {
    ...
    if(order.status !== "available"){
        return reply.status(400).send({message:"Order is already confirmed!!"});
    }
...
export const updateOrderStatus=async(req,reply)=>{
  try {
   ...
   if(order.deliveryPartner.toString() !== userId){
    return status(403).send({message:"Unauthorized!!"});
   }
   order.status=status;
